export type Command = 'extractor' | 'collector' | 'colonizer' | 'hunter' | 'bomber' | 'defense' | 'sell' | 'pause' | 'slow' | 'normal';
...
export class Input {
  ...
  constructor(private canvas: HTMLCanvasElement) {
    ...
    window.addEventListener('keyup', (event) => {
      const command = ({ c: 'colonizer', h: 'hunter', b: 'bomber', e: 'extractor', l: 'collector', d: 'defense', s: 'sell', ' ': 'pause' } as Record<string, Command>)[event.key.toLowerCase()];
